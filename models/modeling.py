...
class DualTransformers(nn.Module):
    def __init__(self, margin=0.3, margin2=0.95, dataset='PS', temp=1.0, train_classes=None,
        ...
        self.IE = ImageEncoder(num_v=num_v)

        self.TE = TextEncoder(num_t=num_t)

        self.vae = FGLA(img_dim=512, text_dim=512, latent_dim=512)

    def forward(self, img_encoding, text_encoding, text_lens):

        img_ld, img_re = self.IE(img_encoding)
        text_ld, text_re = self.TE(text_encoding, text_lens)
        img_ae_emb, text_ae_emb, fgla_loss = self.vae(img_encoding, img_ld, text_ld)

        return img_re, text_re, img_ae_emb, text_ae_emb, fgla_loss
